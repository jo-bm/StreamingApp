...
class CatalogTestCase(unittest.TestCase):
    ...
    def test_get_404(self):
        url = "{}/nonexistent".format(BASE_URL)
        res = requests.get(url)
        self.assertEqual(res.status_code, 404)

    def test_get_file_list(self):
        for url in FILE_LIST_URLS:
            res = requests.get("{}{}".format(BASE_URL, url))
            self.assertEqual(res.status_code, 200)

    def test_get_episodes(self):
        url = "{}/file_list/Black_Lagoon".format(BASE_URL)
        res = requests.get(url, headers=self.headers)
        self.assertEqual(res.status_code, 200)
        for i in range(5):
            episode_number = str(random.randint(1, 10)).zfill(2)
            episode_name = "Black_Lagoon_S01E{}".format(episode_number)
